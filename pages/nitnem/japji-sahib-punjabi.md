...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const baniData = {
        punjabi: {
            sections: [
                ...
                { id: 'pauri-10', title: 'ਪਉੜੀ ੧੦', text: 'ਸੁਣਿਐ ਸਤੁ ਸੰਤੋਖੁ ਗਿਆਨੁ ॥ ਸੁਣਿਐ ਅਠਸਠਿ ਕਾ ਇਸਨਾਨੁ ॥ ਸੁਣਿਐ ਪੜਿ ਪੜਿ ਪਾਵਹਿ ਮਾਨੁ ॥ ਸੁਣਿਐ ਲਾਗੈ ਸਹਜਿ ਧਿਆਨੁ ॥ ਨਾਨਕ ਭਗਤਾ ਸਦਾ ਵਿਗਾਸੁ ॥ ਸੁਣਿਐ ਦੂਖ ਪਾਪ ਕਾ ਨਾਸੁ ॥੧੦॥' },
        ...
        english: {
            ...
            'jap-slok': "True In The Primal Beginning. True Throughout The Ages. True Here And Now. O Nanak, Forever And Ever True. ||1||",
            ...
            'pauri-12': "The state of the believer cannot be described. One who tries to describe it, regrets the attempt. No paper, no pen, no scribe can sit and contemplate the state of the believer. Such is the Name of the Immaculate Lord. Only one who has faith comes to know such a state in the mind. ||12||",
    ...
    function createSectionHTML(section) {
        ...
        let html = `<div class="line-group" id="${section.id}">
                        <p class="gurmukhi">${section.text}</p>
                        <p class="translation ${translationClass}">${translationText}</p>
                    </div>`;
        if (section.id === 'mool-mantar') {
            html += `<div class="jap-heading">॥ ਜਪੁ ॥</div>`;
        }
        return html;
    }

    // MODIFIED: This function now loads all content at once.
    function loadAllContent() {
        const fragment = document.createDocumentFragment();
        allSections.forEach(section => {
            const sectionWrapper = document.createElement('div');
            // Using innerHTML is fine here as we are constructing the full element string
            sectionWrapper.innerHTML = createSectionHTML(section);
            // Move all children from the wrapper to the fragment
    ...
    // MODIFIED: This function now populates navigation and adds listeners.
    ...
    // --- Initial setup ---
    // REMOVED lazy loading. Now we just load content and then set up navigation.
    loadAllContent();
    generateNavigation();
});
</script>
